...
import {getCourseWithAttendanceKey} from "./courseMethods";

Meteor.methods({

  updateComment(attendanceKey, bookingId, comment) {
    const course = getCourseWithAttendanceKey(attendanceKey)
    const booking = getBooking(bookingId)
    if (booking.courseId == course._id) {
      updateBooking({$set: {comment: comment}}, booking._id, true)
    } else {
      throw new Meteor.Error("Booking " + bookingId + " belongs to course " + booking.courseId + ", but attendanceKey " + attendanceKey + " belongs to course " + course._id)
    }
  },
  ...
  setBookingPaid(bookingId, paid) {
    ...
    Bookings.update({_id: bookingId}, {$set: {paid: paid, modifiedTime: new Date()}})
  }
...
export function updateBooking(modifier, _id, validate) {
  ...
  modifier.modifiedTime = new Date()
...
export function getUnpaidBookingsForCourse(courseId, includeRecentlyModified) {
  let orBookingsModifiedAfterDate = null
  if (includeRecentlyModified) {
    orBookingsModifiedAfterDate = new Date()
  }
  console.log("getUnpaidBookingsForCourse - orBookingsModifiedAfterDate = " + orBookingsModifiedAfterDate)

  return Bookings.find(
    {courseId: courseId, $or: [{paid: false}, {modifiedTime: {$gt: orBookingsModifiedAfterDate}}]},
