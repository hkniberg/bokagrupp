  ...
  addUser(userToAdd) {
    ...
    if (Meteor.isServer) {
      if (!isSuperUser(this.userId)) {
        ...
        //OK I am orgAdmin.
        if (userToAdd.role != "orgAdmin") {
          throw new Meteor.Error("As orgAdmin, you are only allowed to add other orgAdmin roles, not " + userToAdd.role)
        }
