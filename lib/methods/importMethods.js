...
Meteor.methods({
  importData(orgId) {
    console.assert(orgId, "orgId is missing")

    if (Meteor.isServer) {
      assertAdmin(this.userId)
      return importData(orgId)
    }
  }
})
...
function importData(orgId) {
  ...
  courses.forEach((pgCourse) => {
    ...
    const courseReport = importCourse(orgId, pgCourse)
...
function importCourse(orgId, pgCourse) {
  console.assert(orgId, "orgId is missing")
  ...
  log("Importing course " + pgCourse.short_name + " to org " + orgId + "....")
