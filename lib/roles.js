...
export function isAdmin(userId, orgId) {
  ...
  if (isSuperUser(userId) || isOrgAdmin(userId, orgId) || isCourseAdmin(userId, orgId)) {
    return true
  } else {
    return false
  }
...
export function isCourseAdmin(userId, orgId) {
  console.assert(userId, "userId wasn't given")
  return Roles.userIsInRole(userId, 'courseAdmin', orgId)
}
...
export function assertAdmin(userId, orgId) {
  if (!isAdmin(userId, orgId)) {
    console.log("User " + userId + ", must be superUser or courseAdmin or orgAdmin, and isn't!")
    throw new Meteor.Error("You don't have permission for this")
  }
}